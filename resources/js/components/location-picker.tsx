...
export function LocationPicker({
    ...
    const { googleApiKey } = usePage<SharedData>().props;
    const isControlled = onValueChange !== undefined;
    const [internalValue, setInternalValue] = useState<google.maps.LatLngLiteral | undefined>(defaultValue);
    ...
    const displayValue = actualValue ? `${actualValue.lat.toFixed(4)}, ${actualValue.lng.toFixed(4)}` : placeholder;
        ...
        <Dialog>
            ...
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>Double-click to place a marker, then drag it to adjust the location.</DialogDescription>
                </DialogHeader>
                <APIProvider apiKey={googleApiKey}>
                    <GoogleMap
                        ...
                        {...props}
                        className={cn('min-h-full overflow-hidden rounded-md border', className)}
                        disableDoubleClickZoom
                        onDblclick={(ev) => !disabled && handleChange(ev.detail.latLng)}
                        styles={isDarkMode ? darkMapStyle : lightMapStyle}
                    >
                        {actualValue && (
                            <Marker position={actualValue} draggable={!disabled} onDragEnd={(ev) => !disabled && handleChange(ev.latLng?.toJSON())} />
                        )}
                    </GoogleMap>
                </APIProvider>
                <DialogFooter className="sm:justify-start">
                    ...
                    {actualValue && (
                        <Button variant="destructive" onClick={() => handleChange(undefined)} disabled={disabled}>
                            Clear Selection
                        </Button>
                    )}
                </DialogFooter>
